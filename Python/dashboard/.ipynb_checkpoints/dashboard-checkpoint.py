...
pangan = [
    ...
    'Cabai Merah', 'Cabai Rawit', 'Daging Sapi', 
    'Telur Ayam', 'Daging Ayam', 'Minyak Goreng', 
    'Gula Pasir'
]
...
def load_data_norm():
    df = pd.read_csv("https://github.com/ayalya/Harga-Bahan-Pangan/raw/main/Data/csv/data_mingguan_norm.csv")
...
def diagram_garis_bapok(df):
    ...
    if on:#opsi=='Normalisasi':
        df=data_norm
        y = 'Nilai (Scale)'
        data_log = df.melt(id_vars=['Minggu ke'], value_vars=pangan,
                           var_name="Komoditas", value_name=y)
...
def alignment_and_counting_dtw():
    ...
    # Pilih komoditas
    with col1:
        st.markdown('<h3 class="subheading">Ukur Jarak DTW</h3>', unsafe_allow_html=True)
        names1 = st.selectbox("Time-series 1", list(pangan), index=list(pangan).index("Cabai Merah"))
        names2 = st.selectbox("Time-series 2", list(pangan), index=list(pangan).index("Cabai Rawit"))
    ...
    fig.add_trace(go.Scatter(y=s2, mode='lines', name=names2, line=dict(color='orange')))
...
def pilih_jmlh_cluster():
    ...
    """
    st.markdown('<h6 class="sub-subheading">Parameter Klaster</h6>', unsafe_allow_html=True)
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        jmlh_cluster = [2,3,4,5]
        c = st.selectbox("Jmlh Klaster (c)", list(jmlh_cluster), index=list(jmlh_cluster).index(3))
    with col2:
...
def visualisasi_hasil_cluster(df_data, df_cluster,
...
st.markdown('<h2 class="big-font">CLUSTERING HARGA BAHAN PANGAN PROVINSI BANTEN</h2>', unsafe_allow_html=True)
...
with colhead2:
    ...
    st.markdown('<h6 class="sub-subheading">Derajat Keanggotaan</h6>', unsafe_allow_html=True)
